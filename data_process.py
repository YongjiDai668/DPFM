...
class FewRelProcessor():
    ...
    def set_task_order(self, filename, index,filename2=None):
        ...
        with open(os.path.join(self.args.data_dir, 'FewRel', filename2), 'r', encoding='utf-8') as file:
            self.task_order2 = json.loads(file.read())
        self.task_order2 = self.task_order2[index]
        for i in range(10):
            self.relations_description += self.task_order2["T" + str(i + 1)]
    ...
    def _read_data_from_order(self, filename):  ##实验数据的取得
        ret, length = [], []
        with open(os.path.join(self.args.data_dir, 'FewRel', f'Exp{self.order_index}', filename), 'r',
                  encoding='utf-8') as file:
            data = json.loads(file.read())
        re_data = re_construct_data(data, self.tokenizer, self.BERTtokenizer)
        data = re_data
    ...
    def get(self):
        if self.read_from_order:
            ...
            for i in range(self.task_num):
                ...
                relation_description = self.relations_description[
                                       i * self.relnum_per_task: (i + 1) * self.relnum_per_task]
                ...
                taskdatas.append({
                    ...
                    'test': test,
                    'train_len': train_len,
                    'relations_description': relation_description
                })
        else:
            data = self._read()
...
class tacredProcessor():
    ...
    def set_task_order(self, filename, index, filename2=None):
        ...
        for i in range(10):
            self.relations += self.task_order["T" + str(i + 1)]
        for rel in self.relations:
            self.rel2id[rel] = self.relations.index(rel)  ##rel2id是存储关系的索引  rel2id[per;other_family]==1
        print(f"Experiment Num {index}")
        assert len(self.relations) == len(self.rel2id)

        with open(os.path.join(self.args.data_dir, 'tacred', filename2), 'r', encoding='utf-8') as file:
            self.task_order2 = json.loads(file.read())
        self.task_order2 = self.task_order2[index]
        for i in range(10):
            self.relations_description += self.task_order2["T" + str(i + 1)]
    ...
    def _read_data_from_order(self, filename):  ###对json数据进行处理
        ret, length = [], []
        with open(os.path.join(self.args.data_dir, 'tacred', f'Exp{self.order_index}', filename), 'r',
                  encoding='utf-8') as file:
            data = json.loads(file.read())
        re_data = re_construct_data(data, self.tokenizer, self.BERTtokenizer)
        data = re_data
        for key, value in data.items():
            for instance in value:
                ...
                instance["input_ids"] = instance["tokens"][:tail]  ##去0化后的输入
                instance["h_index"] = instance["input_ids"].index(30522)  ##E11所在的位置
                instance["t_index"] = instance["input_ids"].index(30524)  ##E21所在的位置
                instance["relation"] = key
                instance["label"] = self.rel2id[key]
                ret.append(instance)
            length.append(len(value))
    ...
    def get_ralation_description(self):
        descriptions = [self.tokenizer.encode(' '.join(relation[relation.index(":") + 1:].split('_'))) for relation in
                        self.relations]
        return descriptions
...
def re_construct_data(data, tokenizer, BERTtokenizer):
    re_data = {}
    ...
    for key, value in data.items():
        ...
        for instance in value:
            # 解码tokens以获取文本
            decoded_text = BERTtokenizer.decode(instance["tokens"], skip_special_tokens=False)
            # 使用特殊标记找到实体1和实体2
            entity1 = decoded_text.split("[E11]")[1].split("[E12]")[0]
            entity2 = decoded_text.split("[E21]")[1].split("[E22]")[0]
            # 构建新的文本格式
            new_text = f"[CLS] {entity1} [MASK] {entity2} [SEP] " + decoded_text.replace(
                "[CLS]", "")
            # 将新文本编码为tokens
            new_tokens = tokenizer.encode(new_text, add_special_tokens=False)
            # 重建instance，这里添加了新的tokens和原始的relation
            re_instance = {"tokens": new_tokens, "relation": instance["relation"]}
            re_data[key].append(re_instance)
    return re_data