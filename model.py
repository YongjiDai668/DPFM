...
class BertEncoder(nn.Module):
    def __init__(self, args, tokenizer, encode_style="emarker"):
        ...
        self.model = AutoModel.from_pretrained('princeton-nlp/sup-simcse-bert-base-uncased').to(args.device)
    ...
    def forward(self, input_ids, attention_mask , h_index= None, t_index=None,labels = None,):    ##可能要改一下别的输入传入
        plm_output = self.model(input_ids, attention_mask=attention_mask)['last_hidden_state']
        cls_embedding = plm_output[:, 0, :]
        # else:
        if h_index is not None:
            ...
            # mask_hidden= torch.stack([plm_output[i, MASK_index[i], :] for i in range(len(MASK_index))], dim=0)
...
class Classifier(nn.Module):
    def __init__(self, args, hidden_dim, label_num, prev_classifier=None):
        ...
        self.classifier = nn.Linear(hidden_dim, label_num, bias=False) ######bias =ture
        self.loss_fn = CrossEntropyLoss()

        # if prev_classifier is not None:
        #     self.classifier.weight.data[:row, :] = prev_classifier.weight.data
